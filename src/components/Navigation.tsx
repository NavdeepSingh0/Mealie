...
const Navigation = () => {
  ...
  // Filter out AI Chat for mobile navigation bar
  const mobileLinks = links.filter(link => link.to !== "/ai-chat");
  const displayLinks = isMobile ? mobileLinks : links;
    ...
    <>
      ...
      {isMobile && (
        <Link
          to="/ai-chat"
          className={cn(
            "fixed bottom-24 right-6 z-50 flex items-center justify-center w-14 h-14 rounded-full shadow-lg transition-all hover:scale-110 md:hidden",
            isAIChatActive
